// jpx.js — standalone JPEG2000 (J2K codestream) decoder for GRIB2 data
// representation template 5.40 (used by RAP and other JPEG2000-packed NCEP
// products). Adapted from Mozilla PDF.js (Apache-2.0): src/core/jpx.js and
// src/core/arithmetic_decoder.js, with the PDF.js-specific dependencies inlined
// and the final assembly changed to return raw integer samples (with the J2K
// DC level shift) instead of clamped 8-bit pixels, as GRIB2 fields can carry
// more than 8 bits per value.
//
// Upstream: https://github.com/mozilla/pdf.js (v2.16.105)

/* eslint-disable */

// --- inlined helpers (from pdf.js core_utils.js / shared/util.js) ---
class BaseException extends Error {}
function log2(x) { return x <= 0 ? 0 : Math.ceil(Math.log2(x)); }
function readUint16(data, offset) { return (data[offset] << 8) | data[offset + 1]; }
function readUint32(data, offset) {
  return (((data[offset] << 24) | (data[offset + 1] << 16) |
    (data[offset + 2] << 8) | data[offset + 3]) >>> 0);
}
function warn(msg) { /* console.warn('jpx:', msg); */ }
function info(_msg) {}
function unreachable(msg) { throw new Error(msg); }

// --- MQ arithmetic decoder (pdf.js arithmetic_decoder.js) ---
...
// --- JPEG2000 codestream decoder (pdf.js jpx.js) ---
/* Copyright 2012 Mozilla Foundation
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
...
class JpxImage {
  ...
  parseCodestream(data, start, end) {
    ...
    this.tiles = transformComponentsRaw(context);
...
// Assemble decoded tiles into raw integer samples (single-component grayscale,
// as GRIB2 JPEG2000 always is). Applies the DC level shift for unsigned
// components so the values match the originally packed integers.
function transformComponentsRaw(context) {
  const siz = context.SIZ;
  const components = context.components;
  const componentsCount = siz.Csiz;
  const out = [];
  for (let i = 0, ii = context.tiles.length; i < ii; i++) {
    const tile = context.tiles[i];
    // GRIB2 uses a single component; honor only component 0.
    const transformed = transformTile(context, tile, 0);
    const comp = components[0];
    const shift = comp.isSigned ? 0 : 1 << (comp.precision - 1);
    const src = transformed.items;
    const items = new Int32Array(src.length);
    for (let j = 0, jj = src.length; j < jj; j++) {
      // Reversible coefficients are integers; round irreversible ones.
      items[j] = (src[j] | 0) === src[j] ? src[j] + shift : Math.round(src[j]) + shift;
    }
    out.push({
...
// Decode a raw J2K codestream (GRIB2 section 7 payload for DRT 5.40) into a
// flat Int32Array of samples in raster order (row-major, top-left origin) —
// the same layout as the PNG path in grib2.js.
export function decodeJ2K(bytes) {
  const image = new JpxImage();
  image.parse(bytes);
  const tiles = image.tiles;
  const width = image.width;
  const height = image.height;
  const samples = new Int32Array(width * height);
  for (const tile of tiles) {
    const tw = tile.width;
    for (let y = 0; y < tile.height; y++) {
      let dst = (tile.top + y) * width + tile.left;
      let s = y * tw;
      for (let x = 0; x < tw; x++) samples[dst++] = tile.items[s++];
    }
  }
